...
class XMLRPC(API):
    ...
    def patch_list(
        ...
        if state is not None:
            ...
            if state_id == 0:
                sys.stderr.write(
                    f"Note: No State found matching {state}*, ignoring filter\n"
                )
            else:
        ...
        if project is not None:
            ...
            if project_id == 0:
                sys.stderr.write(
                    f"Note: No Project found matching {project}, ignoring filter\n"
                )
            else:
        ...
        if delegate is not None:
            ...
            if len(delegate_ids) == 0:
                sys.stderr.write(
                    f"Note: Nobody found matching *{delegate}*\n"
                )
            else:
    ...
    def patch_get(self, patch_id):
        ...
        if patch == {}:
            raise exceptions.APIError(
                f'Unable to fetch patch {int(patch_id)}; does it exist?'
            )
    ...
    def patch_get_mbox(self, patch_id):
        ...
        if len(mbox) == 0:
            raise exceptions.APIError(
                f'Unable to fetch mbox for patch {int(patch_id)}; does it exist?'
            )
    ...
    def patch_set(
        ...
        if state:
            ...
            if state_id == 0:
                raise exceptions.APIError(
                    f'No State found matching {state}*'
                )
                sys.exit(1)
        ...
        try:
            ...
            raise exceptions.APIError(
                f'Error updating patch: {f.faultString}'
            )
    ...
    def check_create(
        ...
        try:
            ...
            raise exceptions.APIError(
                f'Error creating check: {f.faultString}'
            )
